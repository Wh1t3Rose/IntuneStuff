...
Install-Module -Name PSAppDeployToolkit -RequiredVersion 4.1.0 -Scope CurrentUser -Force
Import-Module -Name PSAppDeployToolkit -RequiredVersion 4.1.0 -Force
```

## Create PSADT Package Directory
```powershell
# Create a new package template folder using the installed PSADT module
$packageRoot = Join-Path (Get-Location) 'Zoom Outlook Plugin'
