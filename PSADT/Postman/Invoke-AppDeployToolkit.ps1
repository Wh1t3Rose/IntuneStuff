...
$ExeInstallerName   = 'Postman.exe'
$ExeUninstallerName = 'Update.exe'
...
if ($adtSession.EnableWelcomePrompt) {
    ...
    # Remove Postman user directory after uninstall
    $postmanUserDir = Join-Path $env:LocalAppData 'Postman'
    if (Test-Path $postmanUserDir) {
        Remove-Item -Path $postmanUserDir -Recurse -Force -ErrorAction SilentlyContinue
        Write-ADTLogEntry -Message "Deleted $postmanUserDir during Post-Uninstall." -Severity 1
    }
