...
$adtSession = @{
    ...
    AppScriptDate = '2026-01-26'
    AppScriptAuthor = '<Tyler Cox>'
    RequireAdmin = $false

    # Enable prompts
    EnableWelcomePrompt     = $true
    EnableCompletionPrompt  = $true
...
function Install-ADTDeployment
{
    ...
    if (-not [string]::IsNullOrWhiteSpace($ExeInstallerName)) {
        ...
        # Prefer a configured DefaultMsiFile if present
        ...
        elseif ($adtSession.UseDefaultMsi) {
            # If UseDefaultMsi is enabled but DefaultMsiFile not explicitly set, attempt to run the Zero-Config flow
            $ExecuteDefaultMSISplat = @{ Action = $adtSession.DeploymentType; FilePath = $adtSession.DefaultMsiFile }
            if ($adtSession.DefaultMstFile) {
                $ExecuteDefaultMSISplat.Add('Transforms', $adtSession.DefaultMstFile)
            }
            ...
            Show-ADTInstallationPrompt -Message "No installer configured and no Zero-Config MSI available." -Icon Error -NoWait
...
if ($adtSession.EnableWelcomePrompt) {
    ...
    # 1 & 2: Respect explicit $ExeUninstallerPath
    if (-not [string]::IsNullOrWhiteSpace($ExeUninstallerPath)) {
        ...
        elseif (Test-Path -LiteralPath $ExeUninstallerPath -PathType Container -ErrorAction SilentlyContinue) {
            ...
            # If it looks like a path ending with slash, try combining with name
    ...
    # 3: Use $ExeUninstallerName from the package files folder
    ...
    # 4: Search for any uninstaller-like exe in the package files
    ...
    # 5: If resolved use it, otherwise fall back to MSI
