...
$adtSession = @{
    ...
    AppScriptDate = '2025-08-26'
    AppScriptAuthor = 'Tyler Cox'
    RequireAdmin = $false

    # Install Titles (Only set here to override defaults set by the toolkit).
    InstallName = 'Lenovo TrackPoint Legacy'
    InstallTitle = 'Lenovo TrackPoint Legacy'
...
function Uninstall-ADTDeployment
{
    ...
    # Define DisplayName of app in Programs and Features
    $appName = "Lenovo ThinkPad TrackPoint Quick Assist"

    # Get the uninstall info
    $app = Get-InstalledApplication -Name $appName

    if ($app) {
        Write-Log "Found $appName installed. Uninstalling..."

        # If MSI-based
        if ($app.IsMsi -eq $true) {
            Execute-MSI -Action Uninstall -Path $app.ProductCode -PassThru
        }
        else {
            # Non-MSI uninstall (EXE string)
            Execute-Process -Path $app.UninstallString -Parameters "/quiet /norestart" -WindowStyle Hidden
        }

        Show-InstallationPrompt -Message "$appName has been successfully uninstalled." -ButtonRightText "OK"
    }
    else {
