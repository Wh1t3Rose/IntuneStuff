...
function Install-ADTDeployment
{
    ...
    Start-ADTProcess -FilePath "Files\WinSCP-6.5.5-Setup.exe" -ArgumentList '/VERYSILENT /ALLUSERS /NORESTART /LOG="C:\ProgramData\Logs\Software\WinSCPInstaller.txt"'
...
function Uninstall-ADTDeployment
{
    ...
    ## <Perform Uninstallation tasks here>
        ...
        if (-not (Get-Variable -Name 'ExeInstallerName' -Scope Script -ErrorAction SilentlyContinue)) { $ExeInstallerName = $null }
        if (-not (Get-Variable -Name 'ExeInstallArgs' -Scope Script -ErrorAction SilentlyContinue)) { $ExeInstallArgs = $null }
    Start-ADTProcess -FilePath $envProgramFilesX86\WinSCP\unins000.exe -ArgumentList '/VERYSILENT /ALLUSERS /NORESTART /LOG="C:\ProgramData\Logs\Software\WinSCPInstaller.txt"'
