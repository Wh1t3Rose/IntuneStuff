...
$adtSession = @{
    ...
    AppProcessesToClose = @('WinRar')  # Example: @('excel', @{ Name = 'winword'; Description = 'Microsoft Word' })
...
If (Test-Path "$envProgramFiles\WinRAR\Uninstall.exe") {
    Start-ADTProcess -FilePath "$envProgramFiles\WinRAR\Uninstall.exe" -ArgumentList "/S" -ErrorAction "SilentlyContinue"
}
If (Test-Path "$envProgramFiles(x86)\WinRAR\Uninstall.exe") {
    Start-ADTProcess -FilePath "$envProgramFiles(x86)\WinRAR\Uninstall.exe" -ArgumentList "/S" -ErrorAction "SilentlyContinue"
    ...
    ## Start-ADTMSIProcess -FilePath "Files\7z2501-x64.msi" -ArgumentList '/x' -ErrorAction SilentlyContinue
