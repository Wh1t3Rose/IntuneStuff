...
$ExeInstallerName   = ''
$ExeUninstallerName = 'UninstallerTool.exe'
...
$ExeUninstallArgs = "-product snagit"
...
if ($adtSession.EnableWelcomePrompt) {
    ...
    # Uninstall Any Existing Versions of Snagit 2023 using PSADT 4.1.x functions
    $Uninstaller = Get-ChildItem -Path $adtSession.DirFiles -Include UninstallerTool.exe -File -Recurse -ErrorAction SilentlyContinue
    if ($Uninstaller -and $Uninstaller.Count -gt 0) {
        Write-ADTLogEntry -Message "Found $($Uninstaller.FullName), now attempting to uninstall TechSmith Snagit 2023."
        Start-ADTProcess -FilePath $Uninstaller.FullName -ArgumentList "-product snagit" -WindowStyle Hidden
        Start-Sleep -Seconds 5
    } elseif (-not $Uninstaller) {
        # Uninstall Any Existing Versions of Snagit 2023 (MSI) - Delete Library
        Remove-ADTMsiApplications -Name 'Snagit 2023' -AddParameters 'TSC_DATA_STORE=1'
    }
    ...
    # Delete TechSmith Start Menu folder after uninstall
    $startMenuFolder = 'C:\ProgramData\Microsoft\Windows\Start Menu\Programs\TechSmith'
    if (Test-Path $startMenuFolder) {
        try {
            Remove-Item -Path $startMenuFolder -Recurse -Force
            Write-ADTLogEntry -Message "Deleted folder: $startMenuFolder"
        } catch {
            Write-ADTLogEntry -Message "Failed to delete folder: $startMenuFolder. $_" -Severity 2
        }
    }
