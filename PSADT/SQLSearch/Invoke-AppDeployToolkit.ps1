...
$adtSession = @{
    ...
    AppProcessesToClose = @( @{ Name = 'SQL Search'; Description = 'SQL Search' })
...
function Install-ADTDeployment
{
    ...
    Start-ADTMSIProcess -FilePath "$dirFiles\SQL Search_3.9.3.6351_x86.msi" -Transforms "$dirFiles\SQL Search_3.9.3.6351_x86.mst" -ArgumentList '/qn'
...
function Uninstall-ADTDeployment
{
    ...
    ## <Perform Uninstallation tasks here>
        ...
        if (-not (Get-Variable -Name 'ExeInstallerName' -Scope Script -ErrorAction SilentlyContinue)) { $ExeInstallerName = $null }
        if (-not (Get-Variable -Name 'ExeInstallArgs' -Scope Script -ErrorAction SilentlyContinue)) { $ExeInstallArgs = $null }
    Start-ADTMSIProcess -Action Uninstall -FilePath "$dirFiles\SQL Search_3.9.3.6351_x86.msi"
