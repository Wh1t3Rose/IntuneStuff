...
$adtSession = @{
    ...
    AppProcessesToClose = @(
    @{ Name = 'WinMerge';  Description = 'WinMerge'  }
    @{ Name = 'WinMergeU'; Description = 'WinMergeU' }
    )
    AppScriptVersion = '1.0.0'
    AppScriptDate = '2026-01-20'
...
function Install-ADTDeployment
{
    ...
    Start-ADTProcess -FilePath "Files\WinMerge-2.16.52.2-x64-Setup.exe" -ArgumentList '/SP- /VERYSILENT /SUPPRESSMSGBOXES /NORESTART'
...
## <Perform Uninstallation tasks here>
    ...
    if (-not (Get-Variable -Name 'ExeInstallerName' -Scope Script -ErrorAction SilentlyContinue)) { $ExeInstallerName = $null }
    if (-not (Get-Variable -Name 'ExeInstallArgs' -Scope Script -ErrorAction SilentlyContinue)) { $ExeInstallArgs = $null }
## Uninstall Any Existing Version of WinMerge

$AppList = Get-ADTApplication -Name 'WinMerge'
...
foreach ($App in $AppList) {
    ...
    if ($App.UninstallString) {
        ...
        if (Test-Path -Path $UninstPath) {
            ...
            Write-ADTLogEntry -Message "Found $($App.DisplayName) ($($App.DisplayVersion)) and a valid uninstall string. Attempting uninstall."

            Start-ADTProcess `
                -FilePath $UninstPath `
                -ArgumentList '/VERYSILENT /NORESTART /LOG=C:\Windows\Logs\Software\WinMerge-Uninstall.log'
