...
function Install-ADTDeployment
{
    ...
    ## Show Welcome Message, close Internet Explorer if required, allow up to 5 deferrals, verify there is enough disk space to complete the install, and persist the prompt.
    Show-ADTInstallationWelcome `
  -CloseProcesses "iexplore", "winword", "excel", "powerpnt", "outlook", "onenote", "msaccess", "visio", "project", "lync", "teams" -AllowDefer -DeferTimes 5  -CheckDiskSpace  -PersistPrompt

    
    ## Show Progress Message (with a message to indicate the application is being uninstalled).
    Show-ADTInstallationProgress -StatusMessage 'Please Wait...'

    Remove-Item -Path "C:\ProgramData\VACO\InstalledApps\Office-Latest.tag" -Force -ErrorAction SilentlyContinue
...
Start-ADTProcess -FilePath "setup.exe" -ArgumentList "/configure Install.xml"
    ...
    $adtSession.InstallPhase = "Post-$($adtSession.DeploymentType)"
        ...
        New-Item -Path "C:\ProgramData\VACO\InstalledApps\Office-Latest.tag" -ItemType File -Force
...
function Uninstall-ADTDeployment
{
    ...
    Start-ADTProcess -FilePath 'Files\ODT.exe' -ArgumentList "/configure uninstall.xml"
}
