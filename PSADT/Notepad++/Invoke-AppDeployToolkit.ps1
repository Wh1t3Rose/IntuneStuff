...
function Install-ADTDeployment
{
    ...
    # Tagging: create tag root and tag file
    ...
    $nppFolder = Join-Path $env:ProgramFiles 'Notepad++'
    $nppUninstaller = Join-Path $nppFolder 'uninstall.exe'
    if (Test-Path $nppFolder) {
        if (Test-Path $nppUninstaller) {
            Start-ADTProcess -FilePath $nppUninstaller -ArgumentList '/S'
            Start-Sleep -Seconds 10
        } else {
            # If uninstall.exe is not found, try MSI uninstall if available
            if ($adtSession.UseDefaultMsi) {
                $ExecuteDefaultMSISplat = @{ Action = 'Uninstall'; FilePath = $adtSession.DefaultMsiFile }
    ...
    #Start-ADTProcess -FilePath "Files\npp.8.9.1.Installer.x64.msi" -ArgumentList '/S'
