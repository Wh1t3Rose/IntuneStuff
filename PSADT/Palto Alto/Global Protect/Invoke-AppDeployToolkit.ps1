...
function Install-ADTDeployment
{
    ...
    $msiFile = Get-ChildItem -Path $adtSession.DirFiles -Filter '*.msi' -File -ErrorAction SilentlyContinue | Select-Object -First 1
    if (-not $msiFile) {
        Write-ADTLogEntry -Message "No MSI found in $($adtSession.DirFiles)" -Severity 3
        Show-ADTInstallationPrompt -Message "No MSI found in $($adtSession.DirFiles)" -ButtonRightText 'OK' -Icon Error -NoWait
        Close-ADTSession -ExitCode 69002
    }

    Start-ADTMsiProcess -Action Install -FilePath $msiFile.FullName -ArgumentList '/quiet /norestart'
