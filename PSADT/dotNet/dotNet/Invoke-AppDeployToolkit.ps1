...
function Install-ADTDeployment
{
    ...
    Remove-Item -Path "C:\ProgramData\VACO\InstalledApps\dotNet_8.0.19.tag" -Force -ErrorAction SilentlyContinue
    ...
    $dotNetRuntimes = @(
        @{ Name = "Microsoft .NET Runtime - 5.0.17 (x64)"; File = "dotnet-runtime-5.0.17-win-x64.exe" },
        @{ Name = "Microsoft .NET Runtime - 6.0.36 (x64)"; File = "dotnet-runtime-6.0.36-win-x64.exe" },
        @{ Name = "Microsoft .NET Runtime - 7.0.20 (x64)"; File = "dotnet-runtime-7.0.20-win-x64.exe" }
    )

    foreach ($runtime in $dotNetRuntimes) {
        $app = Get-ADTApplication -Name $runtime.Name
        if ($app) {
            Start-ADTProcess -FilePath "Files\$($runtime.File)" -ArgumentList "/install /quiet /norestart" -ErrorAction SilentlyContinue
            $dotNetRemoved = $true
        }
    }

    if ($dotNetRemoved) {
        Start-ADTProcess -FilePath "Files\windowsdesktop-runtime-8.0.19-win-x64.exe" -ArgumentList "/install /quiet /norestart"
    }
...
New-Item -Path "C:\ProgramData\VACO\InstalledApps\dotNet_8.0.19.tag" -ItemType File -Force
...
function Uninstall-ADTDeployment
{
    ...
    Start-ADTProcess -FilePath "Files\windowsdesktop-runtime-8.0.19-win-x64.exe" -ArgumentList "/uninstall /quiet /norestart"
