...
$adtSession = @{
    ...
    AppProcessesToClose = $initialProcessesToClose
...
function Install-ADTDeployment
{
    ...
    $Source = "C:\Windows\Logs\Software"
    $Target = "C:\ProgramData\Microsoft\IntuneManagementExtension\Logs\PSADT"
...
if ($adtSession.EnableWelcomePrompt) {
    ...
    $JunctionPath = "C:\ProgramData\Microsoft\IntuneManagementExtension\Logs\PSADT"
    $SourcePath   = "C:\Windows\Logs\Software"
    ...
    if (Test-Path $JunctionPath) {
        ...
        if ($Item.Attributes -band [IO.FileAttributes]::ReparsePoint) {
            Write-Output "Junction detected. Removing..."
            [IO.Directory]::Delete($JunctionPath, $false)
            Write-Output "Junction removed successfully."
        }
        else {
            Write-Output "Path exists but is NOT a junction. No action taken."
        }
    }
    else {
        Write-Output "Junction path does not exist. Nothing to remove."
    }
