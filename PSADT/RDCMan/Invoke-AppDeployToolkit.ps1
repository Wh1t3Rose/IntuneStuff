...
$adtSession = @{
    ...
    # App variables.
    AppVendor = 'Microsoft'
    AppName = 'RDCMan' # Setting this disables Zero-Config MSI support; set to $null or '' to enable Zero-Config MSI if an MSI file is present in the Files folder.
    AppVersion = ''
    AppArch = ''
    AppLang = 'EN'
    AppRevision = '01'
...
function Install-ADTDeployment
{
    ...
    $rdcTargetDir = 'C:\RDCMan'
    $rdcSourcePath = Join-Path -Path $adtSession.DirFiles -ChildPath 'RDCMan.exe'

    if (-not (Test-Path -LiteralPath $rdcTargetDir)) {
        New-Item -Path $rdcTargetDir -ItemType Directory -Force | Out-Null
    }

    if (-not (Test-Path -LiteralPath $rdcSourcePath)) {
        Write-ADTLogEntry -Message "Source file missing: $rdcSourcePath" -Severity 3
        Show-ADTInstallationPrompt -Message "Source file missing: $rdcSourcePath" -ButtonRightText 'OK' -Icon Error -NoWait
        Close-ADTSession -ExitCode 69010
    }

    Copy-Item -Path $rdcSourcePath -Destination $rdcTargetDir -Force
    Write-ADTLogEntry -Message "Copied RDCMan.exe to $rdcTargetDir"
