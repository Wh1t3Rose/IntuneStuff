...
function Install-ADTDeployment
{
    ...
    Remove-Item -Path "C:\ProgramData\VACO\InstalledApps\autorunregkey.tag" -Force -ErrorAction SilentlyContinue
	...
	# Set NoDriveTypeAutoRun to 255 (0xFF) to disable AutoRun on all drives
	$explorerPath = "HKLM:\Software\Microsoft\Windows\CurrentVersion\Policies\Explorer"

	# Create the registry path only if it doesn't exist
	if (-not (Test-Path $explorerPath)) {
	    New-Item -Path $explorerPath -Force | Out-Null
	}

	# Set or update the NoDriveTypeAutoRun value
	New-ItemProperty -Path $explorerPath -Name "NoDriveTypeAutoRun" -Value 255 -PropertyType DWord -Force | Out-Null
...
New-Item -Path "C:\ProgramData\VACO\InstalledApps\autorunregkey.tag" -ItemType File -Force
...
function Uninstall-ADTDeployment
{
	...
	# Remove NoDriveTypeAutoRun to restore default behavior (if it exists)
	$explorerPath = "HKLM:\Software\Microsoft\Windows\CurrentVersion\Policies\Explorer"
	if (Test-Path "$explorerPath\NoDriveTypeAutoRun") {
	    Remove-ItemProperty -Path $explorerPath -Name "NoDriveTypeAutoRun" -Force
	}
