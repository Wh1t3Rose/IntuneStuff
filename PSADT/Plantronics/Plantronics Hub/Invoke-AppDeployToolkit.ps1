...
$adtSession = @{
    ...
    InstallName = 'Plantronics Hub'
    InstallTitle = 'Plantronics Hub'
...
Show-ADTInstallationProgress
    ...
    ## Remove Any Existing Versions of Plantronics Hub Software (EXE)
    Uninstall-ADTApplication -Name 'Plantronics Hub Software' -ApplicationType 'EXE' -ArgumentList "/uninstall /quiet /norestart /log `"$((Get-ADTConfig).Toolkit.LogPath)\$($adtSession.AppName).log`"" -ErrorAction SilentlyContinue
    ...
    ## Perform Plantronics Hub Software Installation
    $files = Get-ChildItem -Path "$($adtSession.DirFiles)" -File -Recurse -ErrorAction SilentlyContinue
    $exePath = $files | Where-Object { $_.Name -match 'PlantronicsHubInstaller.exe' }
    if ($exePath.Count -gt 0) {
        Show-ADTInstallationProgress -StatusMessage 'Installing the Plantronics Hub Software. Please Wait...'
        Start-ADTProcess -FilePath "$($exePath.FullName)" -ArgumentList "/install /quiet /norestart /log `"$((Get-ADTConfig).Toolkit.LogPath)\$($adtSession.AppName).log`"" -WindowStyle 'Hidden'
