...
$adtSession = @{
    ...
    AppProcessesToClose = @('csc_ui')  # Example: @('excel', @{ Name = 'winword'; Description = 'Microsoft Word' })
    AppScriptVersion = '1.0.0'
    AppScriptDate = '2025-09-11'
    AppScriptAuthor = '<Tyler Cox>'
    RequireAdmin = $false

    # Install Titles (Only set here to override defaults set by the toolkit).
    InstallName = 'VACO Cisco Anyconnect VPN'
    InstallTitle = 'VACO Cisco Anyconnect VPN'
...
function Install-ADTDeployment
{
    ...
    Start-ADTMsiProcess -Action 'Install' -FilePath 'Files\cisco-secure-client-win-5.1.0.136-core-vpn-webdeploy-k9.msi' -ArgumentList '/norestart /passive'
    ...
    New-ADTFolder -LiteralPath "$envProgramData\Cisco\Cisco Secure Client\"

    New-ADTFolder -LiteralPath "$envProgramData\Cisco\Cisco Secure Client\VPN"

    New-ADTFolder -LiteralPath "$envProgramData\Cisco\Cisco Secure Client\VPN\Profile"

    Copy-ADTFile -Path "$($adtSession.dirFiles)\Profile\*" -Destination "$envProgramData\Cisco\Cisco Secure Client\VPN\Profile" -Recurse -ContinueFileCopyOnError
