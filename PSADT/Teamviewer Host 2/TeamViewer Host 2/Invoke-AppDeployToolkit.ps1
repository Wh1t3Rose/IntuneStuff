...
# TeamViewer Host ProductCode
$teamViewerProductCode = '{E9067C23-CFD2-4AED-B789-44426B80CE80}'

# Registry keys where MSI ProductCode could be registered
$regKey        = "HKLM:\Software\Microsoft\Windows\CurrentVersion\Uninstall\$teamViewerProductCode"
$regKeyWow6432 = "HKLM:\Software\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall\$teamViewerProductCode"

if ((Test-Path $regKey) -or (Test-Path $regKeyWow6432)) {
    Write-ADTLogEntry -Message "TeamViewer Host detected with ProductCode $teamViewerProductCode. Uninstalling..." -Severity 1 -ScriptSection 'Pre-Install'
    
    # Uninstall using MSI
    Start-ADTMsiProcess -Action 'Uninstall' -ProductCode $teamViewerProductCode -PassThru
    
    Write-ADTLogEntry -Message "TeamViewer Host uninstall completed." -Severity 1 -ScriptSection 'Pre-Install'
}
else {
    Write-ADTLogEntry -Message "TeamViewer Host not detected. Skipping uninstall." -Severity 1 -ScriptSection 'Pre-Install'
}
...
Test-Path "Files\TeamViewer_Host.msi"
Test-Path "Files\HighspringSettings.tvopt"

$configID         = '69v62f7'
$apiToken         = '26957618-e7Q8GOhdu7hVgZmvtJoL'
$assignmentOptions = '--grant-easy-access'

Start-ADTMsiProcess -Action 'Install' -FilePath "Files\TeamViewerhostinstaller.msi" -AdditionalArgumentList @(
    '/qn',
    "CUSTOMCONFIGID=$configID",
    "APITOKEN=$apiToken",
    "ASSIGNMENTOPTIONS=`"$assignmentOptions`"",
    "SETTINGSFILE=`"$($adtSession.DirFiles)\HighspringSettings.tvopt`""
)
...
New-Item -Path "C:\ProgramData\VACO\InstalledApps\TeamViewer Host - 15.68.5.0.tag" -ItemType File -Force
