...
function Install-ADTDeployment
{
    ...
    Show-ADTInstallationWelcome -CloseProcesses -CloseProcessesCountdown 60

    ## Show Progress Message (with a message to indicate the application is being uninstalled).
    Show-ADTInstallationProgress -StatusMessage 'Removing Any Existing Versions of .Net SDK < 8.0. Please Wait...'
	...
	# Get .NET info
	Start-ADTProcess -FilePath "$envProgramFilesX86\dotnet\dotnet.exe" -ArgumentList "--info" -ErrorAction SilentlyContinue -PassThru

	# List installed .NET SDKs and runtimes
	Start-ADTProcess -FilePath "$envProgramFilesX86\dotnet-core-uninstall\dotnet-core-uninstall.exe" -ArgumentList "list" -ErrorAction SilentlyContinue -PassThru

	# Remove all .NET Core components below 8.0
	Start-ADTProcess -FilePath "$envProgramFilesX86\dotnet-core-uninstall\dotnet-core-uninstall.exe" -ArgumentList "remove --all-below $latestversion --sdk --force --verbosity d -y" -ErrorAction SilentlyContinue -PassThru
	Start-ADTProcess -FilePath "$envProgramFilesX86\dotnet-core-uninstall\dotnet-core-uninstall.exe" -ArgumentList "remove --all-below $latestversion --runtime --force --verbosity d -y" -ErrorAction SilentlyContinue -PassThru
	Start-ADTProcess -FilePath "$envProgramFilesX86\dotnet-core-uninstall\dotnet-core-uninstall.exe" -ArgumentList "remove --all-below $latestversion --aspnet-runtime --force --verbosity d -y" -ErrorAction SilentlyContinue -PassThru
	Start-ADTProcess -FilePath "$envProgramFilesX86\dotnet-core-uninstall\dotnet-core-uninstall.exe" -ArgumentList "remove --all-below $latestversion --hosting-bundle --force --verbosity d -y" -ErrorAction SilentlyContinue -PassThru

	# Remove NuGet fallback folder
	Remove-Folder -FilePath "$envProgramFiles\dotnet\sdk\NuGetFallbackFolder" -ErrorAction SilentlyContinue
...
New-Item -Path "C:\ProgramData\VACO\InstalledApps\dotnet-core-uninstall.tag" -ItemType File -Force
