...
function Install-ADTDeployment
{
    ...
    Remove-Item -Path "C:\ProgramData\VACO\InstalledApps\dotnet-removalscript.tag" -Force -ErrorAction SilentlyContinue
...
foreach ($path in $uninstallPaths) {
    ...
    foreach ($subKey in $subKeys) {
        ...
        if ($displayName -and $uninstallStr) {
            if ($displayName -match "Microsoft\s+\.NET\s+SDK\s+([4-7]\.\d+)") {
                if ($uninstallStr -match "msiexec") {
                    Start-ADTProcess -FilePath "msiexec.exe" -ArgumentList "/x $($props.PSChildName) /quiet /norestart" -WindowStyle Hidden -Wait
                }
                else {
                    Start-ADTProcess -FilePath "cmd.exe" -ArgumentList "/c $uninstallStr /quiet /norestart" -WindowStyle Hidden -Wait
                }
            }
        }
    }
...
New-Item -Path "C:\ProgramData\VACO\InstalledApps\dotnet-removalscript.tag" -ItemType File -Force -ErrorAction SilentlyContinue
