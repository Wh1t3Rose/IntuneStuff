...
$InstallName  = 'Adobe Creative Cloud Desktop'
$ProcessName  = 'Adobe Creative Cloud.exe'
...
$ExeInstallerName   = 'Build\setup.exe'
...
function Install-ADTDeployment
{
    ...
    $buildDir = Join-Path -Path $adtSession.DirFiles -ChildPath 'Build'
    $setupPath = if ($ExeInstallerName) {
        Join-Path -Path $adtSession.DirFiles -ChildPath $ExeInstallerName
    }

    if ($setupPath -and (Test-Path -LiteralPath $setupPath)) {
        Start-ADTProcess -FilePath $setupPath -ArgumentList $ExeInstallArgs
    }
    else {
        $msiFile = Get-ChildItem -Path $buildDir -Filter '*.msi' -File -ErrorAction SilentlyContinue | Select-Object -First 1
        if ($msiFile) {
            Start-ADTMsiProcess -Action $adtSession.DeploymentType -FilePath $msiFile.FullName
        }
        else {
            Write-ADTLogEntry -Message "No setup.exe or MSI found in $buildDir" -Severity 3
            Show-ADTInstallationPrompt -Message "No setup.exe or MSI found in $buildDir" -ButtonRightText 'OK' -Icon Error -NoWait
