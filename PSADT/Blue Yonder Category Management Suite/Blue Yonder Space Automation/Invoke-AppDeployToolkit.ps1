...
$InstallName  = 'Space Automation'
$ProcessName  = 'SpaceAutomation.exe'
...
$adtSession = @{
    # App variables.
    # App variables.
    AppVendor = 'Blue Yonder'
    AppName = 'Space Automation'
    AppVersion = ''
    AppArch = ''
    AppLang = 'EN'
    AppRevision = '01'
...
# Arguments for MSI installation (Space Automation Pro)
$MsiInstallerName = 'SpaceAutomationPro.msi'
$MsiInstallerPath = ''
$MsiInstallLog = "C:\Windows\Logs\Software\$InstallName-MSI-Install.log"
$MsiUninstallLog = "C:\Windows\Logs\Software\$InstallName-MSI-Uninstall.log"
$MsiInstallArgs = "INSTALLDIR=`"C:\BlueYonder\Category Management\Space Automation Professional`" VSPLATFORM=2 SKUNAME=Community /qn"
$MsiUninstallArgs = "/qn /l*v `"$MsiUninstallLog`""
...
function Install-ADTDeployment
{
    ...
    ## Handle MSI installation if $MsiInstallerName is set
    if ($MsiInstallerName) {
        ...
        $resolvedMsiPath = $MsiInstallerName
        $msiSearchRoot = if ([string]::IsNullOrWhiteSpace($MsiInstallerPath)) { $adtSession.DirFiles } else { $MsiInstallerPath }
        # If not an absolute path, search $adtSession.DirFiles
        if (-not ([System.IO.Path]::IsPathRooted($resolvedMsiPath))) {
            $candidate = Join-Path -Path $msiSearchRoot -ChildPath $resolvedMsiPath
            if (Test-Path -LiteralPath $candidate) {
                $resolvedMsiPath = $candidate
            } else {
                # Try to find the MSI recursively in DirFiles
                $found = Get-ChildItem -Path $msiSearchRoot -Filter $MsiInstallerName -Recurse -File -ErrorAction SilentlyContinue | Select-Object -First 1
                if ($found) { $resolvedMsiPath = $found.FullName }
            }
        }
        Start-ADTMsiProcess -FilePath $resolvedMsiPath -ArgumentList $MsiInstallArgs
    }
    ...
    if ($adtSession.UseDefaultMsi)
    {
        ...
        #Start-ADTMsiProcess -FilePath "Files\SpaceAutomationPro.msi" -ArgumentList 'VSPLATFORM=2 SKUNAME=Community /qn'
    }
...
if ($adtSession.EnableWelcomePrompt) {
    ...
    ## Handle MSI uninstallation if $MsiInstallerName is set
    if ($MsiInstallerName) {
        $adtLogDir = 'C:\Windows\Logs\Software'
        if (-not (Test-Path -LiteralPath $adtLogDir)) {
            New-Item -Path $adtLogDir -ItemType Directory -Force | Out-Null
        ...
        Start-ADTMsiProcess -Action Uninstall -FilePath $resolvedMsiPath -ArgumentList $MsiUninstallArgs
    }

    ## Handle MSI uninstallation directly
    if ($adtSession.UseDefaultMsi)
    {
        $msiPath = $adtSession.DefaultMsiFile
        $cmd = "msiexec /x `"$msiPath`" /qn"
        Write-ADTLogEntry -Message "Running: $cmd"
        Start-ADTProcess -FilePath "msiexec.exe" -ArgumentList "/x", $msiPath, "/qn"
    }
