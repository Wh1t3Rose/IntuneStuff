...
$adtSession = @{
    # App variables.
    AppVendor = 'VACO'
    AppName = 'Personal Teams Removal'
    AppVersion = ''
    AppArch = ''
    AppLang = 'EN'
    AppRevision = '01'
    ...
    InstallName = 'Personal Teams Removal'
    InstallTitle = 'Personal Teams Removal'
...
## Remove Microsoft Teams (Personal Edition)

Write-ADTLogEntry -Message "Starting removal of Microsoft Teams (Personal Edition)."

$TeamsPersonal = Get-AppxPackage -AllUsers -Name "MicrosoftTeams"

if (-not $TeamsPersonal) {
    Write-ADTLogEntry -Message "Microsoft Teams (Personal Edition) not found for any user." -Severity 1
} else {
    # Kill the Teams Personal process if running
    Stop-Process -Name "msteams" -Force -ErrorAction SilentlyContinue
    Write-ADTLogEntry -Message "Stopped msteams.exe process if it was running." -Severity 1

    # Remove the app for all users
    Get-AppxPackage -AllUsers -Name "MicrosoftTeams" | Remove-AppxPackage -AllUsers
    Write-ADTLogEntry -Message "Removed Microsoft Teams (Personal Edition) using Remove-AppxPackage -AllUsers." -Severity 1
}
