...
# --- App Info ---
$InstallName  = 'SQL Server Developer Edition'
...
if (-not $adtSession.InstallWelcomeMessage) {
    $adtSession.InstallWelcomeMessage = "Setup will install $($adtSession.InstallName). Please save your work before continuing."
}
if (-not $adtSession.UninstallWelcomeMessage) {
    $adtSession.UninstallWelcomeMessage = "$($adtSession.InstallName) will be removed from this device."
}
if (-not $adtSession.CompletionMessage) {
    $adtSession.CompletionMessage = "$($adtSession.InstallName) has been successfully installed."
}
...
function Install-ADTDeployment {
    ...
    # Run setup.exe with ConfigurationFile.ini using DirFiles
    Write-ADTLogEntry -Message "INI Exists: $(Test-Path "$($adtSession.DirFiles)\ConfigurationFile.ini")"
Start-ADTProcess -FilePath "$($adtSession.DirFiles)\setup.exe" -ArgumentList "/ConfigurationFile=`"$($adtSession.DirFiles)\ConfigurationFile.ini`" /IACCEPTSQLSERVERLICENSETERMS"
    ...
    if ($adtSession.EnableCompletionPrompt) {
        Show-ADTInstallationPrompt -Message $adtSession.CompletionMessage -ButtonRightText 'OK' -Icon Information -NoWait
    }
}
...
if ($adtSession.EnableWelcomePrompt) {
    ...
    if (-not $ExeUninstallerResolved -and -not [string]::IsNullOrWhiteSpace($adtSession.InstallName)) {
        ...
        # 3) If $ExeUninstallerName is set, check Files directory for the named file
        # 4) Search Files directory recursively for any EXE with 'uninstall'/'unins'/'uninstaller' in the name
    ...
    if (-not $ExeUninstallerResolved -and -not [string]::IsNullOrWhiteSpace($ExeUninstallerName)) {
        $filesDir = "C:\Users\TylerCox\OneDrive - Highspring\Packaging\!Ready\SQL Server Dev Standard\Files"
        $candidate = Join-Path -Path $filesDir -ChildPath $ExeUninstallerName
    ...
    if (-not $ExeUninstallerResolved) {
        try {
            $filesDir = "C:\Users\TylerCox\OneDrive - Highspring\Packaging\!Ready\SQL Server Dev Standard\Files"
            $candidates = Get-ChildItem -Path $filesDir -Filter *.exe -Recurse -File -ErrorAction Stop |
                Where-Object { $_.BaseName -match '(?i)unins|uninstall|uninstaller' }
        }
        catch {
            $filesDir = "C:\Users\TylerCox\OneDrive - Highspring\Packaging\!Ready\SQL Server Dev Standard\Files"
            $candidates = Get-ChildItem -Path $filesDir -Filter *.exe -File -ErrorAction SilentlyContinue |
                Where-Object { $_.BaseName -match '(?i)unins|uninstall|uninstaller' }
        }
