...
$adtSession = @{
    ...
    InstallName = 'Teams Clear Cache'
    InstallTitle = 'Teams Clear Cache'
...
if (Test-Path "$env:APPDATA\Microsoft\Teams\logs.txt") {
    Remove-Item "$env:APPDATA\Microsoft\Teams\logs.txt" -Force -ErrorAction SilentlyContinue
}

## Relaunch Teams

# 1. Per-user install
$TeamsLocal = Join-Path $env:LOCALAPPDATA "Microsoft\Teams\Update.exe"

# 2. Office 365 machine-wide install
$TeamsOffice = "C:\Program Files (x86)\Microsoft\Teams\current\Teams.exe"

# 3. UWP install – AUMID for Teams
$TeamsUWP = "MicrosoftTeams_8wekyb3d8bbwe!App"

if (Test-Path $TeamsLocal) {
    Start-Process $TeamsLocal -ArgumentList "--processStart `"Teams.exe`""
}
elseif (Test-Path $TeamsOffice) {
    Start-Process $TeamsOffice
}

else {
    # Launch UWP Teams via explorer
