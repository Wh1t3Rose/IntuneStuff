...
$adtSession = @{
    ...
    InstallName = 'MS Update kb5067036-24H2'
    InstallTitle = 'MS Update kb5067036-24H2'
...
function Install-ADTDeployment
{
    ...
    ## Start-ADTProcess -FilePath 'Files\setup.exe' -ArgumentList '/S'

    ## Start-ADTMsiProcess -Action 'Install' -FilePath 'Files\Adobe_FlashPlayer_11.2.202.233_x64_EN.msi' -ArgumentList '/QN'
    ...
    # Replace the single problematic line with a safe tag-file creation that uses the session values
    $tagBaseDir = 'C:\ProgramData\VACO\InstalledApps'
    if (-not (Test-Path -Path $tagBaseDir)) { New-Item -Path $tagBaseDir -ItemType Directory -Force | Out-Null }
    
    $installName = if ($adtSession.InstallName) { $adtSession.InstallName } elseif ($adtSession.InstallTitle) { $adtSession.InstallTitle } elseif ($adtSession.AppName) { $adtSession.AppName } else { 'MS_Update_kb5067036_24H2' }
    $safeName = $installName -replace '[^\w\.-]','_'
    $tagFile = Join-Path $tagBaseDir ("{0}.tag" -f $safeName)
    
    New-Item -Path $tagFile -ItemType File -Force | Out-Null
    Write-ADTLogEntry -Message "Created tag file: $tagFile" -Severity 1
...
function Uninstall-ADTDeployment
{
    ...
    ## Uninstall-ADTApplication -Name 'Acrobat' -ApplicationType 'MSI'

    $KB = 'KB5074109'

    if (Get-HotFix -Id $KB -ErrorAction SilentlyContinue) {
        Start-ADTProcess -FilePath 'C:\Windows\System32\wusa.exe' -ArgumentList "/uninstall /kb:$($KB.Replace('KB','')) /quiet /norestart"
    }
