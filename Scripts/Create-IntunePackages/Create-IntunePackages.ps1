...
# Ask if user wants to run upload script
if (-not $DryRun) {
    $uploadResponse = Read-Host "Run .\upload-IntunePackages.ps1? (Y/n)"
    if ([string]::IsNullOrEmpty($uploadResponse) -or $uploadResponse -ieq 'Y') {
        $uploadScript = Join-Path -Path (Split-Path -Parent $MyInvocation.MyCommand.Definition) -ChildPath 'upload-IntunePackages.ps1'
        if (Test-Path -Path $uploadScript) {
            Write-Log "Running upload script: $uploadScript" 'INFO'
            & $uploadScript
        } else {
            Write-Log "Upload script not found at: $uploadScript" 'ERROR'
        }
    } else {
        Write-Log "User chose not to run upload script" 'INFO'
    }
}
