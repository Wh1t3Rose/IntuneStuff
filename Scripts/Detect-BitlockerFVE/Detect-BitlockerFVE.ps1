<#
.SYNOPSIS
Detects whether a BitLocker full-disk encryption (FVE) Group Policy registry key exists.

.DESCRIPTION
Checks for the presence of the Group Policy registry key used for BitLocker
(FVE) policies. Presence of HKLM:\SOFTWARE\Policies\Microsoft\FVE typically
indicates that a BitLocker FDE policy has been configured via Group Policy.
