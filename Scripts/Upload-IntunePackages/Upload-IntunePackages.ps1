...
$configsFolder = Join-Path -Path $scriptRoot -ChildPath '!Packages\!upload\configs'
$LogFile = Join-Path -Path $scriptRoot -ChildPath 'Upload-IntunePackages.log'
...
$DefaultInstallCmd = '.\Invoke-AppDeployToolkit.exe -DeploymentType Install -DeployMode NonInteractive'
$DefaultUninstallCmd = '.\Invoke-AppDeployToolkit.exe -DeploymentType Uninstall -DeployMode NonInteractive'
...
$plannedUploads = @()
$applyAssignmentsToAll = $false
$globalRequiredGroups = @()
$globalAvailableGroups = @()
$globalRequiredGroupIds = @()
$globalAvailableGroupIds = @()
foreach ($p in $pkgs) {
	...
	$answerFile = Join-Path $configsFolder ($p.BaseName + '.answers.json')
	$useAnswers = $false
	...
	# If an answers file exists, ask if user wants to upload this package first
	if (-not $DryRun -and (Test-Path $answerFile)) {
		try {
			Write-Host "Found answers file for '$displayName' at $answerFile. Upload this package? (Y/n) " -ForegroundColor Cyan -NoNewline
			$uploadResp = Read-Host
			if ($uploadResp -and $uploadResp.Trim() -match '^[Nn]') {
				Write-Log "User skipped upload for $displayName (answers file present)" 'INFO'
				continue
			}
		} catch { }
		# Now offer to reuse saved answers
		try {
			Write-Host "Use saved answers? (Y/n) " -ForegroundColor Cyan -NoNewline
			$useResp = Read-Host
			if (-not $useResp -or $useResp.Trim() -match '^[Yy]') {
				try {
					...
					if ($ans.RequiredGroupIds) { $requiredGroupIds = @($ans.RequiredGroupIds) }
					if ($ans.AvailableGroupIds) { $availableGroupIds = @($ans.AvailableGroupIds) }
					if ($ans.ReturnCodes) {
						$returnCodes = @()
	...
	if ($detection.Type -eq 'Unknown') {
		$tagFolder='C:\ProgramData\VACO\InstalledApps'
		if (Test-Path $tagFolder) {
			$t=Get-ChildItem -Path $tagFolder -Filter "*$rawName*.tag" -File -ErrorAction SilentlyContinue | Select-Object -First 1
			if ($t) {
				$detection.Type='TagFile'
				$detection.Value=$t.FullName
				$detection.Version=(Get-Content $t -ErrorAction SilentlyContinue | Select-Object -First 1)
				# Build TagFile detection rule automatically
				if (Get-Command -Name New-IntuneWin32AppDetectionRuleFile -ErrorAction SilentlyContinue) {
					try {
						$fn=[System.IO.Path]::GetFileName($t.FullName)
						$fp=[System.IO.Path]::GetDirectoryName($t.FullName)
						...
						Write-Log "Built TagFile DetectionRule for $($t.FullName)" 'INFO'
					} catch {
						Write-Log "Failed to build TagFile detection rule: $($_.Exception.Message)" 'WARN'
					}
				}
			}
		}
	...
	if ($detection.Type -eq 'Unknown') {
		$resp = Read-Host "Detection not found for $displayName. Enter 'File=C:\path\app.exe' or 'TagFile', or press Enter to skip"
		if ($resp -and $resp -match '^File=(.+)$') {
			$detection.Type = 'File'
			$detection.Value = $matches[1].Trim('"', "'")
		} elseif ($resp -and $resp -match '^TagFile$') {
			$tagName = Read-Host "Enter tag file name (e.g. 7-zip.tag):"
			$detection.Type = 'TagFile'
			$detection.Value = Join-Path 'C:\ProgramData\VACO\InstalledApps' $tagName
			# Always build a detection rule for TagFile
			if (Get-Command -Name New-IntuneWin32AppDetectionRuleFile -ErrorAction SilentlyContinue) {
				try {
					$fn = $tagName
					$fp = 'C:\ProgramData\VACO\InstalledApps'
					$detectionRule = New-IntuneWin32AppDetectionRuleFile -Existence -FileOrFolder $fn -Path $fp -Check32BitOn64System $false -DetectionType 'exists'
					Write-Log "Built TagFile DetectionRule for $($detection.Value)" 'INFO'
				} catch {
					Write-Log "Failed to build TagFile detection rule: $($_.Exception.Message)" 'WARN'
				}
			}
	...
	if (-not $detectionRule) {
		...
		} elseif ($detection.Type -eq 'TagFile' -and (Get-Command -Name New-IntuneWin32AppDetectionRuleFile -ErrorAction SilentlyContinue) -and -not $detectionRule) {
			try {
				$fn = [System.IO.Path]::GetFileName($detection.Value)
				$fp = [System.IO.Path]::GetDirectoryName($detection.Value)
				$detectionRule = New-IntuneWin32AppDetectionRuleFile -Existence -FileOrFolder $fn -Path $fp -Check32BitOn64System $false -DetectionType 'exists'
				...
				Write-Log "TagFile detection rule failed: $($_.Exception.Message)" 'WARN'
			}
		}
	}
	...
	# If no icon found, offer to look in a centralized AppIcons folder or open the browser to search/save one
	if (-not $img -and -not $DryRun) {
		try {
			$appIconsFolder = Join-Path -Path (Split-Path -Parent $UploadFolder) -ChildPath 'AppIcons'
			if (-not (Test-Path -Path $appIconsFolder)) { New-Item -Path $appIconsFolder -ItemType Directory -Force | Out-Null; Write-Log "Created AppIcons folder at $appIconsFolder" 'INFO' }

			   Write-Host "No icon found for '$displayName'. Options: [1] Enter icon filename in $appIconsFolder  [2] Open browser to search and save to $appIconsFolder  [S] Skip" -ForegroundColor Yellow
			   $iconChoice = Read-Host 'Enter 1, 2, or S to skip'
			   if ($iconChoice -and $iconChoice.Trim().ToUpper() -eq '1') {
				   Write-Host "Enter icon filename located in $appIconsFolder (e.g. app.png):" -ForegroundColor Cyan -NoNewline
				   $fname = Read-Host
				   if ($fname) {
					   $candidate = Join-Path -Path $appIconsFolder -ChildPath $fname.Trim()
					   if (Test-Path -Path $candidate) {
						   $img = Get-Item -Path $candidate -ErrorAction SilentlyContinue
						   Write-Log "Using user-selected icon: $candidate" 'INFO'
					   } else {
						   Write-Log "Icon file not found: $candidate" 'WARN'
					   }
				   }
			   } elseif ($iconChoice -and $iconChoice.Trim().ToUpper() -eq '2') {
				   $query = [System.Uri]::EscapeDataString("$displayName icon")
				   $url = "https://www.google.com/search?tbm=isch&q=$query"
				   try { Start-Process $url; Write-Log "Opened browser for image search: $url" 'INFO' } catch { Write-Log "Failed to open browser for image search: $($_.Exception.Message)" 'WARN' }
				   Write-Host "Save an image into: $appIconsFolder then press Enter to continue and use any saved image, or S to skip." -ForegroundColor Cyan -NoNewline
				   $afterBrowser = Read-Host
				   if ($afterBrowser -and $afterBrowser.Trim().ToUpper() -eq 'S') {
					   Write-Log "User skipped icon selection for '$displayName' after browser step" 'INFO'
				   } else {
					   $foundFromIcons = $null
					   try { $foundFromIcons = Find-AppLogo -AppName $displayName -LogosFolder $appIconsFolder } catch {}
					   if (-not $foundFromIcons) { try { $foundFromIcons = Find-AppLogo -AppName $rawName -LogosFolder $appIconsFolder } catch {} }
					   # If still not found, search for any jpg/png with part of the app name
					   if (-not $foundFromIcons) {
						   $searchPattern = $displayName -replace '[^a-zA-Z0-9]', ' '
						   $searchWords = $searchPattern -split '\s+' | Where-Object { $_ }
						   $imgFiles = Get-ChildItem -Path $appIconsFolder -Include *.png,*.jpg,*.jpeg -File -ErrorAction SilentlyContinue
						   $match = $null
						   foreach ($file in $imgFiles) {
							   $lowerName = $file.BaseName.ToLower()
							   foreach ($word in $searchWords) {
								   if ($lowerName -like "*$(($word).ToLower())*") {
									   $match = $file.FullName
									   break
								   }
							   }
						   ...
						   if ($match) { $foundFromIcons = $match }
					   }
					   if ($foundFromIcons) {
						   $img = Get-Item -Path $foundFromIcons -ErrorAction SilentlyContinue
						   $logoPath = $foundFromIcons
						   Write-Log "Found icon in AppIcons after user save: $foundFromIcons" 'INFO'
					   } else {
						   Write-Log "No icon discovered in AppIcons after user save step" 'INFO'
					   }
				   }
			   } elseif ($iconChoice -and $iconChoice.Trim().ToUpper() -eq 'S') {
				   Write-Log "User skipped icon selection for '$displayName'" 'INFO'
			   } else {
				   $foundFromIcons = $null
				   try { $foundFromIcons = Find-AppLogo -AppName $displayName -LogosFolder $appIconsFolder } catch {}
				   if (-not $foundFromIcons) { try { $foundFromIcons = Find-AppLogo -AppName $rawName -LogosFolder $appIconsFolder } catch {} }
		...
		} catch { Write-Log "AppIcons handling failed: $($_.Exception.Message)" 'WARN' }
	}

	if ($img -and (Get-Command -Name New-IntuneWin32AppIcon -ErrorAction SilentlyContinue)) { try { $iconObj = New-IntuneWin32AppIcon -FilePath $img.FullName; Write-Log "Created Icon object from: $($img.FullName)" 'INFO' } catch { Write-Log "Failed creating Icon object from $($img.FullName): $($_.Exception.Message)" 'WARN' } }
	...
	try { 
		if (-not (Test-Path -Path $configsFolder)) { New-Item -Path $configsFolder -ItemType Directory -Force | Out-Null }
		$metaPath = Join-Path $configsFolder ($p.BaseName + '.detection.json')
		$detection | ConvertTo-Json -Depth 4 | Out-File -FilePath $metaPath -Encoding utf8
		Write-Log "Wrote detection metadata to $metaPath" 'INFO'
	} catch { }
	...
	try {
		if ($useAnswers) {
			...
			Write-Host "Uninstall mode for '$displayName' ($($p.Name)): [N]onInteractive or [S]ilent? " -ForegroundColor Cyan -NoNewline
			$uResp = Read-Host "[N]/S"
			if (-not $uResp -or $uResp.Trim() -match '^[Nn]') {
				...
				$uninstallCmd = '.\Invoke-AppDeployToolkit.exe -DeploymentType Uninstall -DeployMode Silent'
	...
	# Per-package assignment group prompts (Required, Available, and explicit Group IDs).
	try {
		if (-not $DryRun) {
			if ($applyAssignmentsToAll) {
				$requiredGroups = @($globalRequiredGroups)
				$availableGroups = @($globalAvailableGroups)
				$requiredGroupIds = @($globalRequiredGroupIds)
				$availableGroupIds = @($globalAvailableGroupIds)
				Write-Log "Using global assignment groups for '$displayName' -> RequiredNames: $([string]::Join(',', $requiredGroups)) ; RequiredIds: $([string]::Join(',', $requiredGroupIds)) ; AvailableNames: $([string]::Join(',', $availableGroups)) ; AvailableIds: $([string]::Join(',', $availableGroupIds))" 'INFO'
			} else {
				Write-Host "Enter Required assignment groups for '$displayName' ($($p.Name)) as comma-separated names (type 'All Users' or press Enter to skip): " -ForegroundColor Yellow -NoNewline
				$reqResp = Read-Host
				if ($reqResp -and $reqResp.Trim() -ne '') {
					$requiredGroups = $reqResp -split ',' | ForEach-Object { $_.Trim() } | Where-Object { $_ -ne '' }
				}

				Write-Host "Enter Required assignment Group IDs for '$displayName' ($($p.Name)) as comma-separated GUIDs (press Enter to skip): " -ForegroundColor Yellow -NoNewline
				$reqIdResp = Read-Host
				if ($reqIdResp -and $reqIdResp.Trim() -ne '') {
					$requiredGroupIds = $reqIdResp -split ',' | ForEach-Object { $_.Trim().Trim('"',"'") } | Where-Object { $_ -ne '' }
				}

				Write-Host "Select Available assignment for '$displayName' ($($p.Name)) - choose: [1] Company-Portal  [2] Company-Portal-Test  [3] All Users  [Enter] Skip: " -ForegroundColor Green -NoNewline
				$availResp = Read-Host "Enter 1, 2, 3, or press Enter to skip"
				switch ($availResp.Trim()) {
					'1' { $availableGroups = @('Company-Portal'); break }
					'2' { $availableGroups = @('Company-Portal-Test'); break }
					'3' { $availableGroups = @('All Users'); break }
					default { $availableGroups = @() }
				}

				Write-Host "Enter Available assignment Group IDs for '$displayName' ($($p.Name)) as comma-separated GUIDs (press Enter to skip): " -ForegroundColor Green -NoNewline
				$availIdResp = Read-Host
				if ($availIdResp -and $availIdResp.Trim() -ne '') {
					$availableGroupIds = $availIdResp -split ',' | ForEach-Object { $_.Trim().Trim('"',"'") } | Where-Object { $_ -ne '' }
				}

				Write-Log "Assignment groups for '$displayName' -> RequiredNames: $([string]::Join(',', $requiredGroups)) ; RequiredIds: $([string]::Join(',', $requiredGroupIds)) ; AvailableNames: $([string]::Join(',', $availableGroups)) ; AvailableIds: $([string]::Join(',', $availableGroupIds))" 'INFO'

				$packageList = ($pkgs | ForEach-Object { $_.Name }) -join ', '
				Write-Host "Apply these assignment settings to ALL packages in this run? Packages: $packageList" -ForegroundColor Cyan -NoNewline
				$applyAllResp = Read-Host " [y/N]"
				if ($applyAllResp -and $applyAllResp.Trim() -match '^[Yy]') {
					$applyAssignmentsToAll = $true
					$globalRequiredGroups = @($requiredGroups)
					$globalAvailableGroups = @($availableGroups)
					$globalRequiredGroupIds = @($requiredGroupIds)
					$globalAvailableGroupIds = @($availableGroupIds)
					Write-Log "User chose to apply assignment settings to all packages in this run." 'INFO'
				}
			}
		} else {
			Write-Log "Dry-run: would prompt for assignment group names and Group IDs for '$displayName' (Available options: Company-Portal, Company-Portal-Test, All Users, or Skip)" 'INFO'
		}
...
foreach ($entry in $plannedUploads) {
	...
	$requiredGroupIds = $entry.RequiredGroupIds
	$availableGroups = $entry.AvailableGroups
	$availableGroupIds = $entry.AvailableGroupIds
	try {
		...
		if ($app -and $app.Id) {
			...
			function Assign-AppGroups {
				param([string]$AppId, [string[]]$Groups, [string[]]$GroupIds, [string]$Intent)
				if ((-not $Groups -or $Groups.Count -eq 0) -and (-not $GroupIds -or $GroupIds.Count -eq 0)) { return }
				...
				if ($GroupIds -and $GroupIds.Count -gt 0) {
					foreach ($groupId in $GroupIds) {
						...
						if ($groupCmdlet) {
							try {
								Add-IntuneWin32AppAssignmentGroup -Include -ID $AppId -GroupID $groupId -Intent $Intent -Notification "showAll" -ErrorAction Stop
								Write-Log "Assigned $Intent GroupID '$groupId' to $displayName" 'INFO'
							} catch {
								Write-Log "Failed to assign $Intent GroupID '$groupId' : $($_.Exception.Message)" 'WARN'
							}
						} else {
							Write-Log "Add-IntuneWin32AppAssignmentGroup not available - cannot assign GroupID '$groupId'" 'WARN'
						}
					}
				}
				...
				foreach ($group in $Groups) {
					...
					if ($group -ieq 'Company-Portal') {
						$resolved = @{ Name = 'Company-Portal'; Id = '93ad1798-db5c-41ec-9085-1f5036f59e25' }
						if ($groupCmdlet) {
							try {
					...
					if ($group -ieq 'Company-Portal-Test') {
						$resolved = @{ Name = 'Company-Portal-Test'; Id = '91b419f1-a2e7-4426-87fc-ee14c514cfa0' }
						if ($groupCmdlet) {
							try {
								Add-IntuneWin32AppAssignmentGroup -Include -ID $AppId -GroupID $resolved.Id -Intent $Intent -Notification "showAll" -ErrorAction Stop
								Write-Log "Assigned $Intent group '$($resolved.Name)' (Id: $($resolved.Id)) to $displayName" 'INFO'
			...
			Assign-AppGroups -AppId $app.Id -Groups $requiredGroups -GroupIds $requiredGroupIds -Intent "required"
			Assign-AppGroups -AppId $app.Id -Groups $availableGroups -GroupIds $availableGroupIds -Intent "available"
		}
	} catch {
...
# Prompt to archive uploaded intunewin files
$archiveFolder = Join-Path -Path $UploadFolder -ChildPath '!Uploaded'
$intunewinFiles = Get-ChildItem -Path $UploadFolder -Filter '*.intunewin' -File -ErrorAction SilentlyContinue

if ($intunewinFiles -and $intunewinFiles.Count -gt 0) {
	$archivePrompt = Read-Host "Archive $($intunewinFiles.Count) uploaded .intunewin file(s) to '$archiveFolder'? (Y/N)"
	
	if ($archivePrompt -match '^[Yy]') {
		...
		# Move intunewin files to archive folder
		try {
			$archiveCount = 0
			foreach ($file in $intunewinFiles) {
				...
				if (Test-Path -LiteralPath $destination) {
					...
					Write-Log "Overwriting existing file: $($file.Name)" 'WARN'
				}
				Move-Item -LiteralPath $file.FullName -Destination $destination -Force
				Write-Log "Archived: $($file.Name)" 'INFO'
				$archiveCount++
			}
			Write-Log "Successfully archived $archiveCount file(s) to $archiveFolder" 'INFO'
		} catch {
			Write-Log "Error archiving files: $($_.Exception.Message)" 'ERROR'
		}
	} else {
		Write-Log "Skipped archiving intunewin files." 'INFO'
	}
}
